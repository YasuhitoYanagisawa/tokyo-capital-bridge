...
data['corr'] = {
    ...
    'gapScan': _gapScan,
    'gapBest': _gapBest,
    # 見出しの数字は国債2年で出しているため、10年だけを基準と書いていた旧文を差し替えた。
    'whyY10': ('国債10年と2年の両方で計算しています。10年は日本の長期金利の代表値で、'
               '銀行の貸出金利・社債の発行条件・不動産の利回り評価まで幅広く参照される「基準の金利」です。'
               '一方、付帯調査によれば中小企業の資金使途は運転資金が64.8%を占め、短期・変動の借入が中心です。'
               'そのため政策金利の先行き予想を映す国債2年のほうが実態に近く、実際に相関も強く出ました。'
               'どちらも中小企業が実際に借りる金利そのものではなく、その土台です。'
               '実際の借入金利は同じ付帯調査にあり、2%台が38.7%で最多、45.2%が「上昇した」と回答しています。'),
    # 旧文は「どの組み合わせでも相関は弱いまま」と書いており、走査で -0.54 を見つける前の
    # 記述がそのまま残っていた。見出しの数字と正面から矛盾していたため書き直した。
    'lagAnswer': ('「時差があるだけでは？」という疑いに答えるため、金利を0〜24か月ずらして総当たりで相関を取り直しました。'
                  '金利の水準と前年同月差、国債10年と2年の計4通りを走査しています。'
                  '同月では r=−0.03 と無関係に見えますが、ずらすほど相関は強まり、'
                  '国債2年・水準では24か月で r=−0.54 に達しました。国債10年でも同じ方向で、24か月で r=−0.40 です。'
                  'ただし24か月は走査の上限であり、そこで頭打ちになっているわけではありません。'
                  'また水準どうしの相関は両系列の趨勢を拾いやすいため、金利と業況DIの両方を前年差に直して検算したところ、'
                  '24〜30か月で r=−0.39 と弱まりますが、符号は残りました。'),
}
